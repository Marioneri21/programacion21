...
const mnuusers=document.getElementById("mnuusers");
 ...
 btnpost.addEventListener("click", ()=>{
  ...
  const btnborrame=document.getElementById("btnborrame");
  btnborrame.addEventListener("click",()=>{
      let c1=document.getElementById("mnusers").value;
      fetch(`http://jsonplaceholder.typicode.com/comments?postId=`+c1)
      .then(response=>response.json())
      .then(json=>{
      let res1="";
      json.forEach(pub=>{
...
document.getElementById("user11").innerHTML=res;
})
})
